...
export class SelfRoleService {
  ...
  public async createSelfRoleMessage(
    guildId: string,
    channelId: string,
    messageId: string,
    message?: string
  ) {
    const newEntity = new SelfRoleMessageEntity({
      guildId,
      channelId,
      messageId,
      message,
  ...
  public async findOrCreateRole(selfRoleMessageId: number, roleId: string) {
    const found = await this._db.manager.findOneBy(SelfRoleEntity, {
      roleId,
      selfRoleMessage: { id: selfRoleMessageId },
    })
    ...
    const newRole = new SelfRoleEntity({
      roleId,
      selfRoleMessage: foundSrMessage,
      interactionId: `sr-btn-${selfRoleMessageId}-${roleId}`,
    })
