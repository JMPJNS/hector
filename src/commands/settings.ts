...
export class SettingsCommands {
  ...
  async language(
    ...
    const userLangSelect = new SelectMenuBuilder()
		...
		const components = []
    const userLangRow = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(userLangSelect)
		components.push(userLangRow)

		if ((interaction.member as GuildMember)?.permissions.has("Administrator")) {
			const guildLangRow = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(guildLangSelect)
