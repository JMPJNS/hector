...
export class InteractionEvents {
  ...
  async interactionCreated([interaction]: ArgsOf<"interactionCreate">, client: Client) {
    // check if interaction id is associated with a self role entity
    if (interaction instanceof ButtonInteraction) {
      const sr = await SelfRoleEntity.findOneBy({ interactionId: interaction.customId })
      if (sr) {
        const role = interaction.guild?.roles.cache.find(x => x.id === sr.roleId)
        ...
        if (member && role) {
          ...
          interaction.reply({ ephemeral: true, content: this._ts.__("SUCCESS") })
        }
      }
    }
  }
